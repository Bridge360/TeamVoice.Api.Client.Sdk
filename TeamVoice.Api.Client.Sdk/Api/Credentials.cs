...
namespace TeamVoice.Api
{
    public class Credentials
    {
        ...
        public Credentials(string AccountKey, string AppKey)
        {
            RijndaelEnhanced key = new RijndaelEnhanced("TEAMVOICE");
            RijndaelEnhanced crypt = new RijndaelEnhanced("$25e!T*Fwg+4.H&");
            data = key.Encrypt(crypt.Encrypt(AccountKey ?? "")
                + "\u0001" + crypt.Encrypt(AppKey ?? ""));
        }

        internal string GetAccountKey(string Key)
        {
            RijndaelEnhanced key = new RijndaelEnhanced("TEAMVOICE");
            RijndaelEnhanced crypt = new RijndaelEnhanced("$25e!T*Fwg+4.H&");
            return crypt.Decrypt(key.Decrypt(data).Split('\u0001')[0]);
        }

        internal string GetAppKey(string Key)
        {
            RijndaelEnhanced key = new RijndaelEnhanced("TEAMVOICE");
            RijndaelEnhanced crypt = new RijndaelEnhanced("$25e!T*Fwg+4.H&");
            return crypt.Decrypt(key.Decrypt(data).Split('\u0001')[1]);
        }
        ...
        /// </remarks>
        private class RijndaelEnhanced
        {
